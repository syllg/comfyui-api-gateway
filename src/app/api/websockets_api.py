...
from src.app.settings.setting import BACKGROUND_REPLACEMENT_WORKFLOW, ANIME_STYLE_WORKFLOW, FACE_SWAP_WORKFLOW, BACKGROUND_REPLACEMENT_REMBG_WORKFLOW, ANIME_STYLE_FACE_SWAP_MERGE_WORKFLOW, MULTI_FACE_SWAP_WORKFLOW, FACE_SWAP_SINGLE_WORKFLOW, BACKGROUND_REPLACEMENT_MASK_WORKFLOW, SNOWY_WORKFLOW, CHIBI_WORKFLOW, HEADSWAP_WORKFLOW
...
def get_images(ws, prompt):
    ...
    # Check all available output nodes instead of hardcoded list
    # This makes the function work with any workflow
    for node_id in history['outputs'].keys():
...
def save_images(images):
    ...
    for node_id in images:
        ...
        for idx, image_data in enumerate(images[node_id]):
            ...
            # Decide file format based on image mode / transparency
            save_kwargs = {}
            ...
            output_path = f"results/output_{client_id}_{node_id}_{idx}{ext}"
...
def snowy(image_path: str, p_prompt: str = None, n_prompt: str = None, random_seed: bool = False):
    try:
        # Validate workflow file
        if not os.path.exists(SNOWY_WORKFLOW):
            raise Exception(f"Workflow file not found: {SNOWY_WORKFLOW}")
            
        # Upload images to ComfyUI server
        uploaded_filename = upload_image(image_path)
        
        try:
            workflow = workflow_cache.get_workflow(SNOWY_WORKFLOW)
            print(f"[DEBUG] Successfully loaded snowy generation workflow from cache")
        except Exception as e:
            print(f"[DEBUG] Error loading workflow from cache: {str(e)}")
            raise Exception(f"Failed to load snowy generation workflow: {str(e)}")
        
        workflow["78"]["inputs"]["image"] = uploaded_filename
        workflow["78"]["inputs"]["upload"] = "image"
        print(f"[DEBUG] Set workflow inputs - image: {uploaded_filename}")

        if p_prompt is not None and p_prompt != "":
            workflow["76"]["inputs"]["text"] = p_prompt
        if n_prompt is not None and n_prompt != "":
            ...
            if node_id in saved_paths and saved_paths[node_id]:
                for path in saved_paths[node_id]:
                    try:
                        with Image.open(path) as img:
                            img.verify()
                            print(f"[DEBUG] Validated snowy generation output image from node {node_id}: {path}")
                            validated_nodes.append(node_id)
                    except Exception as e:
                        print(f"[DEBUG] Invalid image for node {node_id}: {str(e)}")
                ...
                if paths:
                    for path in paths:
                        try:
                            with Image.open(path) as img:
                                img.verify()
                                print(f"[DEBUG] Validated snowy generation output image from fallback node {node_id}: {path}")
                                validated_nodes.append(node_id)
                                break
                        except Exception as e:
                            print(f"[DEBUG] Invalid image for fallback node {node_id}: {str(e)}")
        ...
        print(f"Error in snowy generation: {str(e)}")
        raise

def chibi(image_path: str, random_seed: bool = False):
    try:
        # Validate workflow file
        if not os.path.exists(CHIBI_WORKFLOW):
            raise Exception(f"Workflow file not found: {CHIBI_WORKFLOW}")
            
        # Upload images to ComfyUI server
        uploaded_filename = upload_image(image_path)
        
        try:
            workflow = workflow_cache.get_workflow(CHIBI_WORKFLOW)
            print(f"[DEBUG] Successfully loaded chibi generation workflow from cache")
        except Exception as e:
            print(f"[DEBUG] Error loading workflow from cache: {str(e)}")
            raise Exception(f"Failed to load chibi generation workflow: {str(e)}")
        
        workflow["192"]["inputs"]["image"] = uploaded_filename
        workflow["192"]["inputs"]["upload"] = "image"
        print(f"[DEBUG] Set workflow inputs - image: {uploaded_filename}")

        if random_seed:
            workflow["31"]["inputs"]["seed"] = random.randint(0, 777777777777)
        print("[DEBUG] Executing workflow")
            ...
            if node_id in saved_paths and saved_paths[node_id]:
                for path in saved_paths[node_id]:
                    try:
                        with Image.open(path) as img:
                            img.verify()
                            print(f"[DEBUG] Validated chibi generation output image from node {node_id}: {path}")
                            validated_nodes.append(node_id)
                    except Exception as e:
                        print(f"[DEBUG] Invalid image for node {node_id}: {str(e)}")
                ...
                if paths:
                    for path in paths:
                        try:
                            with Image.open(path) as img:
                                img.verify()
                                print(f"[DEBUG] Validated chibi generation output image from fallback node {node_id}: {path}")
                                validated_nodes.append(node_id)
                                break
                        except Exception as e:
                            print(f"[DEBUG] Invalid image for fallback node {node_id}: {str(e)}")
        ...
        print(f"Error in chibi generation: {str(e)}")
        raise

def headswap(face_image_path: str, body_image_path: str, p_prompt: str = None):
    try:
        # Validate workflow file
        if not os.path.exists(HEADSWAP_WORKFLOW):
            raise Exception(f"Workflow file not found: {HEADSWAP_WORKFLOW}")
            
        # Upload images to ComfyUI server
        uploaded_face_filename = upload_image(face_image_path)
        uploaded_body_filename = upload_image(body_image_path)
        
        try:
            workflow = workflow_cache.get_workflow(HEADSWAP_WORKFLOW)
            print(f"[DEBUG] Successfully loaded chibi generation workflow from cache")
        except Exception as e:
            print(f"[DEBUG] Error loading workflow from cache: {str(e)}")
            raise Exception(f"Failed to load chibi generation workflow: {str(e)}")
        
        workflow["349"]["inputs"]["image"] = uploaded_face_filename
        workflow["349"]["inputs"]["upload"] = "image"
        workflow["343"]["inputs"]["image"] = uploaded_body_filename
        workflow["343"]["inputs"]["upload"] = "image"
        print(f"[DEBUG] Set workflow inputs - image: {uploaded_face_filename}, {uploaded_body_filename}")

        if p_prompt is not None and p_prompt != "":
            workflow["348"]["inputs"]["text"] = p_prompt 
            ...
            if node_id in saved_paths and saved_paths[node_id]:
                for path in saved_paths[node_id]:
                    try:
                        with Image.open(path) as img:
                            img.verify()
                            print(f"[DEBUG] Validated headswap generation output image from node {node_id}: {path}")
                            validated_nodes.append(node_id)
                    except Exception as e:
                        print(f"[DEBUG] Invalid image for node {node_id}: {str(e)}")
                ...
                if paths:
                    for path in paths:
                        try:
                            with Image.open(path) as img:
                                img.verify()
                                print(f"[DEBUG] Validated headswap generation output image from fallback node {node_id}: {path}")
                                validated_nodes.append(node_id)
                                break
                        except Exception as e:
                            print(f"[DEBUG] Invalid image for fallback node {node_id}: {str(e)}")
        ...
        print(f"Error in headswap generation: {str(e)}")
        raise
